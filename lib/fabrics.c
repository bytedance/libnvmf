                                      ...
                                      void *id)
{
    ...
    case NVME_ID_CNS_CTRL:
        ...
        log_debug("sn: %20s, mn: %40s, nn: %d, mdts: %d, vwc: %d, ioccsz: %d, iorcsz: %d, "
                  ...
                  ctrl->sn, ctrl->mn, ctrl->nn, ctrl->mdts, ctrl->vwc, ctrl->ioccsz, ctrl->iorcsz,
                  ctrl->icdoff, ctrl->maxcmd, ctrl->oncs);
        break;
                           ...
                           void *opaque)
{
    ...
    cqe = req->cqe;
    if (cqe->status != NVME_SC_SUCCESS) {
        log_error("ns %s error, status 0x%x\n", is_write ? "write" : "read", le16toh(cqe->status));
        ret = -EIO;
    }
...
static void nvmf_ns_discard_cb(struct nvmf_request *req, void *opaque)
{
    ...
    if (cqe->status != NVME_SC_SUCCESS) {
        log_error("discard failed, queue[%d]tag[0x%x], discard error, status 0x%x, lba"
                  "[%lu, %u]\n",
                  req->queue->qid, req->tag, le16toh(cqe->status), le64toh(range->slba),
                  le32toh(range->nlb));
    } else {
